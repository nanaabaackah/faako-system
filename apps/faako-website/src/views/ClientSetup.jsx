...
export default function ClientSetup() {
  const [values, setValues] = useState(() => deepClone(DEFAULT_VALUES));
  ...
  useEffect(() => {
    setValues(mergeDraft(DEFAULT_VALUES, loadDraft()));
    setDraftReady(true);
  }, []);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setStatus({
        state: "error",
        message: getPublicFormErrorMessage(
          error,
          "Could not submit the client setup form. Please try again.",
        ),
      });
