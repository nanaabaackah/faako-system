...
const buildInvoiceDocumentFinancials = async ({
  ...
  const documents = (result.rows || []).map((row) => ({
    ...
    grandTotal: Number(row.financialSnapshot?.version) >= 1
      ? Number(row.financialSnapshot.totalCents || 0) / 100
      : computeDocumentGrandTotal(row),
  }));
...
const buildDocumentFallbackFilter = ({ enabled, alias, sourceType }) => {
  ...
  return `AND NOT EXISTS (
             ...
             WHERE d."organizationId" = ${alias}."organizationId"
               ...
               AND d."issuedAt" IS NOT NULL
               AND d."businessUnit" = 'REEBS_CORE'
           )`;
};
