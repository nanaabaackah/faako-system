...
const requestPythonAnalytics = async (snapshot, requestId = "") => {
  const serviceUrl = normalizeAnalyticsServiceUrl(
    process.env.FAAKO_ANALYTICS_SERVICE_URL || process.env.REEBS_ANALYTICS_SERVICE_URL
  );
  ...
  try {
    const secret = String(
      process.env.FAAKO_ANALYTICS_SERVICE_SECRET || process.env.REEBS_ANALYTICS_SERVICE_SECRET || ""
    ).trim();
    const analyticsRequest = buildSharedAnalyticsRequest(snapshot);
    const response = await fetch(`${serviceUrl}/api/analytics/reebs/dashboard-insights`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Application-Id": analyticsRequest.context.applicationId,
        "X-Tenant-Id": analyticsRequest.context.tenantId,
    ...
    if (String(envelope?.context?.tenantId) !== String(snapshot.organizationId)) {
  ...
  try {
    ...
    try {
      const requestId = event?.headers?.["x-request-id"]
        ...
        || "";
      analytics = await requestPythonAnalytics(snapshot, requestId);
