...
const ACCOUNT_LINKS = [
  { to: '/login', label: 'Staff login', local: true },
...
function Footer() {
  ...
  const handlePromoSubmit = (event) => {
    ...
    const targetUrl = new URL(buildWebsiteUrl('/contact'));
    if (leadEmail) {
      targetUrl.searchParams.set("leadEmail", leadEmail);
    }
    ...
    <footer className="site-footer" aria-labelledby="footer-heading">
      ...
      <div className="footer-main">
        <div className="footer-brand">
          <a className="footer-logo" href={buildWebsiteUrl('/')}>
            <img
        ...
        <div className="footer-right">
          <div className="footer-columns">
            <nav className="footer-column" aria-label="Main links">
              <h3>Menu</h3>
              {MENU_LINKS.map((link) => (
                <a key={link.to} href={buildWebsiteUrl(link.to)} className="footer-link">
                  {link.label}
                </a>
              ))}
            </nav>
