...
- System Health Phase 2 connects the Phase 1 enterprise UI to an authenticated, capability-gated monitoring backend. The server registry schedules bounded trusted checks, persists minimized results, manages incidents and dependencies, aggregates timelines/latency/uptime/health scores, and allows rate-limited audited administrator checks. The migration must be deployed manually before enabling `MONITORING_ENABLED`; provider targets and worker heartbeats remain server-only.
- System Health Phase 3 adds persisted alert rules/deduplication, controlled incident acknowledgement/assignment/notes/resolution/closure/reopening, structured timelines, encrypted in-app/email channels, ordered escalation, maintenance suppression, response/resolution targets, recovery delivery, unread notifications, and audited PDF/CSV exports. Apply the additive Phase 3 migration before deploying this code and keep `MONITORING_ALERTS_ENABLED=false` until one monitoring instance and its dedicated channel-encryption key are ready. WhatsApp and webhook providers remain deliberately disabled. See `docs/apps/dev-erp/system-health-phase3.md`.
- System Health uses the shared Faako form controls and bubble-and-glass surface language throughout its filters, status history, service details, incident tools, alert rules, channels, and maintenance forms. Monitoring alert and recovery emails use the shared email kit, preserve escaped operational metadata, and link directly to `/system-health?incident=<id>` so the matching incident opens after authentication.
...
- The System Health route polls `/api/monitoring/summary` every 30 seconds while visible and retains its range and filters during refresh. Missing targets remain `Unknown`, not `Down`. Dashboard health summaries retain their existing source while sharing the Phase 1 block-timeline presentation. See `docs/apps/dev-erp/system-health-phase2.md` for API, scheduler, incident, dependency, security, and deployment details.
...
- shared security headers run before the existing 1 MB JSON body parser, so normal API responses and malformed or oversized JSON errors receive the same browser security-header baseline.

## Projects Foundation

Projects are organization-scoped records owned by Dev ERP. The current foundation supports personal and external projects through the authenticated `/api/projects` collection and `/api/projects/:id` detail routes. `GET` lists active projects, `POST` creates a project, and `PATCH` updates project data, stage, or archive state. Archived records remain stored in the same `Project` table and are excluded from active lists unless `includeArchived=true` is explicitly requested. Direct detail reads remain available for an archived project when the authenticated user still has organization access.

The additive `20260716000000_add_project_foundation_fields` migration introduced:

- `startDate`: optional project start date
- `progressPercent`: integer percentage with a default of `0`
- `health`: `ProjectHealth` value with a default of `ON_TRACK`

Supported health values are `ON_TRACK` (On track), `AT_RISK` (At risk), and `BLOCKED` (Blocked). Progress must be a whole number from 0 through 100. Start and due dates must be valid dates or null, and the start date cannot be later than the due date. Existing title, project type, stage, priority, currency, budget, and organization rules continue to apply. Ownership is resolved server-side; the current interface intentionally does not expose an Owner field because this installation uses one project owner.

The Projects interface provides create/edit forms, a responsive Kanban overview, health pills, progress bars, clickable project cards, a single project overview page, server-side search by title or client, combined stage/priority/health/type filters, and archive/restore actions from project details. Archive requests are reversible and idempotent: archiving an already archived project preserves its original archive timestamp. Authentication, the `projects` capability, CSRF checks for writes, and organization scoping continue to be enforced by the backend.

The internal `ProjectTask` persistence foundation and backend are available through nested, authenticated `/api/projects/:projectId/tasks` routes. The backend supports create, active list, detail, update, focused status changes, and reversible archive operations. Every task lookup is anchored to an organization-scoped parent project; assignees must belong to that project organization. Task status and priority values, optional start/due dates, and date ordering are validated. Moving a task to `DONE` sets `completedAt`, reopening it clears `completedAt`, and archived tasks are excluded from active lists and remain read-only except for restoration. Project details include responsive list and six-column board views backed by the same task state, with create/edit forms, status controls, priority and assignee display, start/due dates, archive actions, and loading/error/empty states. Board status moves save through the existing task status API and return to the previous column with a visible error if saving fails. The project detail timeline reuses the existing organization-scoped audit store and exposes the latest 50 project activity entries through `/api/projects/:projectId/activity`. It records project creation, updates, and archive actions plus task creation, updates, assignment, status changes, completion, reopening, and archive actions with actor, task reference, timestamp, and limited change metadata. Activity starts with changes made after this feature is deployed; earlier project history is not backfilled.

The Projects module uses Trello email-to-board delivery. Configure `TRELLO_EMAIL_TO_BOARD_ADDRESS` and `RESEND_API_KEY` only on the server; the private board address, Trello API keys, and tokens are never collected or returned by the frontend. New Dev ERP tasks are emailed once to the Trello board/list selected in Trello's email settings. Successful delivery and bounded errors are stored on the Dev ERP task, and failed deliveries have retry controls. Later task edits remain in Dev ERP and are not re-emailed, which prevents duplicate Trello cards. This is intentionally a one-way integration; card URLs, list moves, and Trello-to-Dev-ERP updates require a future server-side OAuth integration. Dev ERP remains the source of truth.

Current limitations: task movement uses accessible status controls rather than drag-and-drop, and the Projects module still does not include milestones, files, comments, financial tracking beyond the existing budget fields, saved task views, reporting, or permanent project or task deletion. Archived projects do not yet have a dedicated archive browser; they can be restored from their detail URL or managed through the scoped API.

Projects foundation coverage includes form payloads and validation, backend field normalization and serialization, access-scope construction, filters and search, archive safety, capability routing, create/update/list/detail browser workflows, active-list archive behavior, archived-detail access, restore behavior, and mobile filter layout.
