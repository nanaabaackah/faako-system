...
- Settings now uses low-risk shared ERP panel, panel-header, stack, and form-group wrappers from `@faako/ui`. The wrappers preserve the existing `panel-grid`, `panel`, `panel-header`, `stack`, and `form-field` class names, so alert settings, Sync Review behavior, routes, API calls, auth, and storage behavior remain unchanged.
