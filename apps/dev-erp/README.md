...
- System Health keeps HTTP status, response time, final URL, timeout/network classification, and check time for each monitored route. Active incidents receive an evidence-based local runbook even when AI is unavailable. Set the server-only `OPENAI_API_KEY` on the Dev ERP Railway service to enable the protected “Analyze with AI” action; `OPENAI_MODEL` and `OPENAI_TIMEOUT_MS` remain optional overrides.
...
The Projects module supports one encrypted Trello connection per organization. Administrators connect a board and map each Dev ERP task status to a distinct open Trello list without exposing stored credentials back to the browser. Task creation and later title, description, due-date, completion, and status changes create or update one linked Trello card; the card id, URL, last sync source, time, status, and bounded error message remain stored on the Dev ERP task. Outbound failures do not roll back project task changes and are visible with retry controls. `TRELLO_WEBHOOK_BASE_URL` enables a board webhook whose id and exact callback URL are stored with the connection. Incoming requests are verified against the raw request body and Trello application secret, deduplicated by action id, and ignore Dev ERP-originated client identifiers to prevent loops. Supported inbound list moves, due-date changes, card archive actions, and Trello completion flags update only the corresponding linked task fields and record Trello as the source. Comments, members, attachments, checklists, and arbitrary card fields are not synchronized. Dev ERP remains the source of truth.
