...
const ProductDetail: React.FC = () => {
  ...
  const productIsVisible = product ? isPricedProduct(product) : false;
  ...
  useSEOMeta({
    ...
    noIndex: !product || !productIsVisible,
  });
  ...
  const related = products
    .filter((p) => p.category === product.category && p.id !== product.id && isPricedProduct(p))
    .slice(0, 3);
  ...
  const getVariantAvailabilityLabel = (variant: ProductVariant) => {
    const status = normalizeStockStatus(variant.stockStatus);
    const availableQuantity = variant.availableQuantity ?? variant.stockQuantity ?? null;
    const lowStockThreshold = variant.lowStockThreshold ?? product.lowStockThreshold ?? 3;

    if (availableQuantity !== null) {
      if (availableQuantity <= 0 && status !== "preorder") return "Out of stock";
      if (availableQuantity <= lowStockThreshold) return "Few left";
      return "In stock";
    }

    return PRODUCT_STOCK_LABELS[status] || "Unavailable";
  };
