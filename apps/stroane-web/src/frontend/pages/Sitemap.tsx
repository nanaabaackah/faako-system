...
import { categoryOptions, isPricedProduct, products } from "../../data/products";
...
const pricedProducts = products.filter(isPricedProduct);
const pricedProductCategories = new Set(pricedProducts.map((product) => product.category));
const pricedCategoryOptions = categoryOptions.filter(
  (category) => category !== "All" && pricedProductCategories.has(category)
);
...
].filter((group) => group.heading !== "Products" || group.links.length > 0);
