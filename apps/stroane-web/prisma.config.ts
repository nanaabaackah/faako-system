...
const loadEnvironmentConfig = () => {
  ...
  const errorCode = loadedFile.error && "code" in loadedFile.error ? loadedFile.error.code : undefined;
...
const isPrismaGenerateCommand = () =>
  process.argv.some((arg) => String(arg).trim() === "generate");
...
const resolveDatabaseUrl = () => {
  ...
  if (!resolvedUrl) {
    if (isPrismaGenerateCommand()) return PLACEHOLDER_DATABASE_URL;
    ...
    throw new Error(`Missing database URL. Set ${expectedVar} or DATABASE_URL.`);
  }
