...
| Faako Website | React 18/Vite SPA | Public marketing plus signup/onboarding/configuration | Astro for public routes; bounded React islands/app flow | Yes | High | Public and application flows mixed; generic SPA HTML; placeholder auth; direct API assumptions | Inventory route ownership and add HTML/form contract tests, then migrate public-only routes first |
...
| REEBS Website | React 19/Vite SPA | Public rentals, shop, cart, checkout, customer access | Astro + React commerce islands | Yes | High | 73 same-path files shared with portal; 30 exact duplicates; live-API sitemap; cookie/API coupling; backend deps in public app | Extract stable contracts/shared public utilities and add SEO/checkout baselines before migrating routes |
