...
Faako API is the Express backend for the current Faako signup flow. It owns signup and health endpoints, Prisma schema, database helpers, and env-driven runtime configuration.

## Current status

Focused backend service for signup and health checks. Faako Website calls this API through `VITE_API_BASE_URL` or the local Vite `/api` proxy.
...
- Any additional endpoints beyond health and signup until documented and deployed intentionally.
...
- Does this change affect `signup`, `health`, runtime config, Prisma, or website API calls?
