...
- Booking create/edit/status actions can queue offline as `CREATE_BOOKING`, `UPDATE_BOOKING_DETAILS`, and `UPDATE_BOOKING_STATUS`. The queued payload includes the booking reference when present, selected customer reference/details, event date/time, selected items, venue/delivery location, status action, idempotency metadata, user/org scope, and pending status. Sync posts to the existing `/.netlify/functions/bookings` endpoint when online.
