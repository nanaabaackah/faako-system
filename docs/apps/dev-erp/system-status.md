...
- System Health Phase 2 is operational after its additive migration is deployed and `MONITORING_ENABLED=true`. It registers business, API, database, infrastructure, external, and worker services; stores minimized checks/incidents/dependencies; exposes authenticated health APIs; polls safely in the Phase 1 UI; and restricts manual checks to administrators. Unconfigured targets are reported as `Unknown`. The scheduler and manual-check throttle are process-local, no destructive retention cleanup runs, and alerting/public status/SLA/restart features remain out of scope.
