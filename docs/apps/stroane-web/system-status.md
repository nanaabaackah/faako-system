...
- Active portal modules: `/admin` dashboard, `/admin/inventory`, `/admin/orders`, `/admin/receipts`, `/admin/accounting`, `/admin/expenses`, `/admin/crm`, `/admin/directory`, `/admin/team`, `/admin/audit-logs`, and `/admin/profile`. The dashboard includes order/stock business analytics and drilldown modals. Inventory includes stock value, KPI drilldowns, full-width admin table pagination, product creation, product lightbox editing, product autosave, row numbers, row selection, and bulk product publishing/archive/delete-listing actions. Orders includes storefront/manual order review, manual order creation, numbered/selectable rows, order-modal previous/next navigation, autosaved fulfillment edits, delivery-location maps, fulfillment notes, and Paystack initialize/status-refresh actions. Receipts includes automatic/manual receipt review, download, resend, and receipt KPIs. Accounting combines paid orders, receipts, inventory value, supplier costs where available, manual historical lump-sum entries, expense exposure, unpaid expense liabilities, and CSV/Excel-compatible exports. Expenses records paid expenses and unpaid obligations by class/payee/due date, then feeds the accounting calculations. CRM/directory includes customer KPIs, search/status filters, numbered/selectable rows, customer creation, account status, and one-time invite link creation. Team supports user assignment and custom role editing for `ADMIN`/`OWNER`; Audit Logs remain `ADMIN`-only. Profile uses shared ERP fields/actions for personal details and appearance preference.
...
- Security hardening foundation: Stroane backend now reuses shared `@faako/security` API headers, keeps CORS allowlist/trusted proxy controls, applies method-aware global API limits plus route-specific auth/session/admin/inquiry/checkout/Paystack limits, revalidates stock/pricing before payment initialization, and minimizes Paystack provider metadata.
...
- Supplier, standalone product, operations, report, and settings module pages are reset placeholders. Product creation/editing/bulk publishing now happens inside `/admin/inventory`; accounting lives in `/admin/accounting` and `/admin/expenses`; the `/admin` dashboard reads protected product, supplier, inventory, movement, alert, and order APIs for overview signals. The one-time inventory bootstrap remains an explicit CLI operation.
...
- Production backend/database deployment on Railway with Railway Postgres. Staging has its own URL mapping and should use staging-only Railway database/secrets before promotion.
...
- In-memory API rate limiting; current buckets are separated for global reads/writes, auth/session, admin, checkout, inquiry, payment, webhook, and internal alert checks, but Railway/provider-level rate controls are still required before public high-volume checkout.
