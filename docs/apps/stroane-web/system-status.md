...
- Checkout fulfillment capture: delivery orders use a searchable/selectable delivery address field. Pickup orders capture a selected pickup spot plus pickup date and time. The backend stores the fulfillment method in `CommerceOrder.deliveryMethod` and the pickup/delivery date in `CommerceOrder.expectedDeliveryDate`.
...
- Product admin, category admin, supplier admin, report, settings, inquiry lead views, advanced order automation, and inventory-to-fulfillment automation until scope and access control are approved.
