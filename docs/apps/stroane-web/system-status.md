...
- Lightweight internal operations portal: `https://portal.stroanesolutions.com` is the dedicated Cloudflare Pages operational surface. Staff authenticate at `/login`, then `/admin` renders the only active portal dashboard inside the shared `@faako/ui` ERP shell. Module routes such as `/admin/inventory`, `/admin/suppliers`, `/admin/products`, `/admin/operations`, `/admin/orders`, `/admin/reports`, and `/admin/settings` are reset placeholders for the next rebuild.
...
- Product admin, category admin, supplier admin, inventory movement, order review, report, settings, and inquiry lead module views until scope and access control are approved. The current internal portal surface is limited to the protected `/admin` dashboard and reset placeholders.
