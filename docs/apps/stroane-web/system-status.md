...
- Lightweight internal operations portal: staff authenticate separately at `/admin/signin`, then protected `/admin/*` routes render inside the shared `@faako/ui` ERP shell. `/admin/operations` provides order search/list/detail and fulfillment notes, while `/admin/inventory` and `/admin/suppliers` reuse the same private staff session for stock visibility, supplier review, movement history, and admin-only audited stock movements. `/admin/orders` remains a compatibility alias.
...
- Public `/signin` is now customer-only. Private backend `SiteUser` staff credentials use `/admin/signin` and route valid admin/viewer users into the protected `/admin/*` operations portal. Private backend access should remain seeded as one admin and one viewer account until a broader admin account model is approved.
