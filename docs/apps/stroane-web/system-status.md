...
- Inventory operations UI is now available at protected route `/admin/inventory` for stock visibility, supplier review, movement history, and audited movement entry against configured inventory items. `/admin/products` can select preferred suppliers, while initial stock-item setup, full supplier editing, product creation, category editing, bulk editing, and order-to-inventory reservation/deduction are not wired into the UI yet.
