...
- Lightweight internal operations portal: `https://portal.stroanesolutions.com` is the dedicated Cloudflare Pages operational surface. Staff authenticate at `/login`, then protected `/admin/*` routes render inside the shared `@faako/ui` ERP shell. `/admin/operations` provides order search/list/detail and fulfillment notes, while `/admin/inventory` and `/admin/suppliers` reuse the same portal-scoped private staff session for stock visibility, supplier review, movement history, and admin-only audited stock movements. `/admin/orders` remains a compatibility alias.
- Protected product and media operations: `/admin/products` provides searchable/filterable product rows, stock and preferred-supplier visibility, edit-light product copy/pricing/category/tag fields, validated local thumbnail/gallery paths, explicit draft/active/archived publishing, and private preferred-supplier editing. Public catalogue APIs return active rows only and omit supplier, cost, and import-review internals, including during server-side seed fallback.
...
- Inventory operations UI is now available at protected route `/admin/inventory` for stock visibility, supplier review, movement history, and audited movement entry against configured inventory items. `/admin/products` can select preferred suppliers. The one-time inventory bootstrap is an explicit CLI operation; full supplier editing, product creation, category editing, bulk editing, and order-to-inventory reservation/deduction are not wired into the UI yet.
...
- Public sign-in actions hand off to `https://portal.stroanesolutions.com/login`. Private backend `SiteUser` staff credentials route valid admin/viewer users into protected `/admin/*` operations. Staff bearer tokens remain portal-origin `sessionStorage` values; no parent-domain auth cookie is required. Private backend access should remain seeded as one admin and one viewer account until a broader admin account model is approved.
