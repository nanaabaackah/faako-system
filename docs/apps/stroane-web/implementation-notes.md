...
- `backend/src/customerAccounts/routes.js` owns both `/api/customer/*` account routes and `/api/admin/customers*` CRM routes. Customer routes read/write only the authenticated customer's profile/order history. Private CRM reads require `crm.view`; writes and invite generation require `crm.create`/`crm.edit`. `ADMIN` and `OWNER` are elevated across these CRM actions.
- Invite and password-reset tokens are generated as random base64url strings and stored only as SHA-256 hashes. The raw signup URL is returned once when the invite is created/regenerated so staff can copy/share it; reset links are emailed server-side, are not displayed by the storefront, expire after one hour, and each new reset request replaces any prior reset-token hash.
...
- Portal dashboard data loading is permission-aware. Only call inventory/product/supplier/movement/alert APIs for sessions with `inventory.view`, and only call order APIs for sessions with `orders.view`; backend admin APIs remain the authorization boundary.
...
- Protected admin order routes live in `backend/src/ordersAdmin/routes.js` under `/api/admin/orders`. Reads require `orders.view`; writes, manual order creation, Paystack initialize, and Paystack verify require `orders.create`/`orders.edit`. `ADMIN` and `OWNER` are elevated across these order actions.
...
- `/admin/accounting` is the private accounting and analytics hub. It combines paid orders, receipts, inventory stock value, supplier cost data where available, manually entered historical lump-sum accounting entries, and expense-liability analytics from the expense hub.
- `/admin/expenses` is the private expense hub under the accounting permission boundary. It records sales, operational, inventory/supplier, delivery, marketing, tax, payroll, rent/utilities, and other expenses.
- Protected accounting routes live in `backend/src/accounting/routes.js` under `/api/admin/accounting/*`. Reads require `accounting.view`; manual ledger entry and expense creation require `accounting.create`. `ADMIN` and `OWNER` are elevated across these accounting actions.
- `AccountingLedgerEntry` is an additive Prisma model for manual historical balances, adjustments, paid expenses, and unpaid expense liabilities. Deploy migration `20260619000000_add_accounting_ledger_entries` before relying on manual entries in production, and deploy `20260704000000_add_expense_metadata_to_ledger` before using the expense hub metadata fields.
- The accounting overview intentionally separates collected revenue, receivables, paid expenses, unpaid expense liabilities, known stock cost value, retail stock value, liabilities, equity, and estimated net position. These are management analytics, not audited statutory financial statements.
...
- The private order-management UI and admin order routes are part of the active portal surface again. Keep order writes and Paystack actions limited to users with order write permissions; `ADMIN` and `OWNER` are elevated, while custom roles must be explicitly granted the relevant order actions.
