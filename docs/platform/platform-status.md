...
- Shared app update notices now live in `@faako/ui` through `AppUpdateNotice` and are mounted in the browser app shells. They detect changed deployed asset signatures and prompt users to refresh without auto-reloading, so active storefront carts, checkout, forms, admin edits, and offline queues are not interrupted during routine Git deployments. They are not a substitute for backend read-only/maintenance controls during migrations or risky writes.
...
- Dev ERP now has an app-owned persistent monitoring and incident-response foundation for business services, APIs, databases, infrastructure, external providers, and workers. It uses trusted server-only checks, additive indexed monitoring/alert/timeline/maintenance/notification tables, deterministic deduplication, controlled incident lifecycle actions, encrypted in-app/email channels, escalation policies, maintenance suppression, operational response/resolution targets, safe exports, authenticated capabilities, organization boundaries, and visibility-aware UI polling. Scheduling, mutation throttling, and delivery execution remain process-local; WhatsApp/webhooks, multi-instance coordination, public status, formal SLA reporting, and destructive retention cleanup remain pending.
...
- Use `skills/update-project-docs` as the repo-local documentation update checklist after code changes. It keeps app README, app docs, package docs, platform logs, env notes, and stale current-state sections aligned.
