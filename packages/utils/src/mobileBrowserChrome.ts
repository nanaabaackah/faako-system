...
const readCssVariableColor = (element?: Element | null, variableName = BROWSER_CHROME_VARIABLE) => {
...
const ensureThemeColorMeta = () => {
  ...
  const existing = Array.from(document.head.querySelectorAll<HTMLMetaElement>(THEME_COLOR_SELECTOR));
